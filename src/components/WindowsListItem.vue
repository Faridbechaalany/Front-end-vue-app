...
export default {
  ...
  methods: {
    ...
    async switchWindow() {
      let response = await axios.put(`https://farid-bechaalany.cleverapps.io/api/windows/${this.window.id}/switch`);
      let updatedWindow = response.data;
      this.$emit('window-updated', updatedWindow);
    },
    deleteWindow(){
      axios.delete(`https://farid-bechaalany.cleverapps.io/api/windows/${this.window.id}`);
